...
filtered <- tg %>% 
  mutate(ids = name,
         dist_mapk11 = bfs_dist(root = .N()$name == "19094", mode = "out"), 
         dist_mapk1 = bfs_dist(root = .N()$name == "26413", mode = "out"), 
         dist_Pla2g4a = bfs_dist(root = .N()$name == "18783", mode = "out"), 
         dist_Pla2g4b = bfs_dist(root = .N()$name == "211429", mode = "out"), 
         dist_pik3ca = bfs_dist(root = .N()$name == "18706", mode = "out"),
         name = names_to_display[ids]) %>% 
  filter(dist_mapk11 <= 2 & dist_mapk1 <= 2 & dist_Pla2g4a <= 2 & dist_Pla2g4b <= 2 & dist_pik3ca <= 2)
```
...
g <- g %>% mutate(name = names_to_display[ids])

end_cap <- 2
start_cap <- 2
arrow_len <- 1

graph_join(g, filtered, by = c("name" = "name", "ids" = "ids")) %>% 
  ...
  geom_edge_link(aes(filter = (angle == 30), 
                     edge_linetype = lty, 
                     label = edge_label), 
                 end_cap = circle(end_cap, "mm"), 
                 start_cap = circle(start_cap, "mm"), 
                 arrow = arrow(length = unit(arrow_len, "mm"))) +
  geom_edge_link(aes(filter = (angle == 90), 
                     edge_linetype = lty, 
                     label = edge_label),
                 end_cap = circle(end_cap, "mm"), 
                 start_cap = circle(start_cap, "mm"), 
                 arrow = arrow(angle = 90, length = unit(arrow_len, "mm"))) +
  geom_node_point(aes(shape = shape), size = 3, col = "grey80") +
  geom_node_text(aes(label = name), size = 3) +
  scale_edge_linetype_manual(values = c("1" = "solid", "3" = "longdash", "5" = "dotdash")) +
  guides(edge_linetype = "none", shape = "none") +
  theme_graph()
```
