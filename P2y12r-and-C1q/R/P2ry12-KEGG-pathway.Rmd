...
migration_paths <- shortest_paths(graph = ig, 
                                  from = V(ig)$name %in% p2ry12_downstream_genes_entrez, 
...
tg_migration %>% 
  ...
  geom_edge_link(aes(filter = (angle == 90)), 
                 start_cap = circle(1.5, "mm"), 
                 end_cap = circle(3, "mm"), 
                 arrow = arrow(length = unit(1.5, "mm"), angle = 90)) +
  geom_node_point(size = 5, color = "orange") +
  geom_node_text(aes(label = name), size = 3)

ggsave("../res/P2ry12/migration-20210805.tiff")
```
