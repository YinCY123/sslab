...
names_to_display[which(names_to_display == "Calcium cation")]

spath_pvn3 <- shortest_paths(graph = ig, 
                        from = V(ig)[V(ig)$name == "cpd:C00076"], 
                        to = V(ig)$name %in% pvn3_entrez, 
                        output = "epath",
                        mode = "all") %>% 
  .$epath

Gca_path_df <- spath_pvn3 %>% 
  sapply(as_ids) %>% 
  lapply(as.matrix) %>% 
  unlist() %>% 
  as.data.frame() %>% 
...
Gca_path_KEGG <- left_join(Gca_path_df, all_path_df, 
                           by = c("from" = "from", "to" = "to"))

N <- tibble(
  ids = c(Gca_path_KEGG$from, Gca_path_KEGG$to) %>% unique()
) %>% 
  mutate(shape = ifelse(grepl("^cpd:|^gl:|^dr:", ids, ignore.case = F), "22", "21"))

E <- Gca_path_KEGG

tg <- tbl_graph(nodes = N, edges = E, directed = T)

set.seed(123)
tg %>% 
  mutate(names = names_to_display[ids], 
         ncol = ifelse(names %in% pvn3_common_up_genes, "tomato", 
                       ifelse(names %in% pvn3_common_down_genes, "steelblue", 
                              ifelse(names == "Gca", "gold", "gray")))) %>% 
  ggraph(layout = "nicely") +
  geom_edge_link(aes(filter = (angle == 30), label = subtype), 
                 end_cap = circle(3, "mm"), 
                 arrow = arrow(length = unit(1, "mm")),
                 color = "gray40", 
                 angle_calc = "along", 
                 ...
                 label_dodge = unit(1, "mm"), 
                 edge_width = 0.1) +
  geom_edge_link(aes(filter = (angle == 90), label = subtype), 
                 end_cap = circle(3, "mm"), 
                 arrow = arrow(length = unit(1, "mm"), angle = 90), 
                 color = "gray40", 
                 angle_calc = "along", 
                 label_size = 0.7, 
                 label_dodge = unit(1, "mm"), 
                 edge_width = 0.1) +
    geom_edge_link(aes(filter = is.na(angle), label = subtype), 
                 end_cap = circle(3, "mm"), 
                 arrow = arrow(length = unit(1, "mm")),
                 color = "gray40", 
                 angle_calc = "along", 
                 ...
                 label_dodge = unit(1, "mm"), 
                 edge_width = 0.1) +
  geom_node_point(aes(color = ncol, shape = shape), 
                  size = 6, show.legend = F) +
  geom_node_text(aes(label = names), size = 1) +
  scale_color_identity()
