...
spath_Pdgfr <- shortest_paths(ig, 
                            from = V(ig)$name %in% c("18596", "68797"),
                            output = "epath", 
                            mode = "out") %>% 
  .$epath
...
spath_df_Pdgfr_KEGG <- left_join(spath_df_Pdgfr, all_path_df, by = c("from" = "from", "to" = "to")) %>% 
...
g %>% 
  ...
  filter(!is.na(dist_pdgfrb), dist_pdgfrb < 3) %>% 
  activate(edges) %>% 
  ...
  guides(edge_linetype = F, shape = F, col = F) +
  theme_graph()
