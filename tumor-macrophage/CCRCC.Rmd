...
```{r}
cell_meta <- ccrcc[[]] %>% 
    select(region:Sample2, cluster:cell_name)

library(org.Hs.eg.db)
gene_meta <- data.frame(ensembl = rownames(ccrcc))
gene_meta$symbol <- mapIds(org.Hs.eg.db, 
...
ggcells(x = sce.ccrcc[, !(sce.ccrcc$region %in% c("PBMC", "LymphNode"))], 
        mapping = aes(x = cell_name, y = PDGFRB)) +
    geom_violin(aes(color = cell_name, fill = cell_name), show.legend = F) +
    geom_jitter(aes(color = cell_name), alpha = 1/3, size = 0.2, show.legend = F) +
...
percent_mean_expression <- function(object = object, region = region, ...){
    ...
    p <- list()
    for(i in region){
        mean_expression <- object[grepl("^NT5E", rownames(object)), object$region == i] %>% 
            logcounts() %>% 
            as.matrix() %>% 
            apply(MARGIN = 1, FUN = function(x){tapply(X = x, INDEX = object[, object$region == i]$cell_name, FUN = mean)}) %>% 
            as.data.frame() %>% 
            rownames_to_column("cell") %>% 
            gather(key = "Gene", value = "mean_expression", -cell)
        
        percent_expression <- object[grepl("^NT5E", rownames(object)), object$region == i] %>% 
            logcounts() %>% 
            as.matrix() %>% 
            apply(MARGIN = 1, FUN = function(x){tapply(X = x, INDEX = object[, object$region == i]$cell_name, FUN = function(x){mean(x > 0) * 100})}) %>% 
            as.data.frame() %>% 
            rownames_to_column("cell") %>% 
            gather(key = "Gene", value = "percent_expression", -cell)
    ...
    ggsave("/home/yincy/git/Data/Kidney/ccRCC/res/expression_NT5E.tiff", 
           width = 10, height = 10, limitsize = F)
}
...
```{r}
rownames(sce.ccrcc) %>% grep("^Nt5e", x = ., ignore.case = T, value = T)
```
