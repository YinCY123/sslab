...
pooled %>% 
  ...
  geom_segment(aes(x = "Corn Oil", y = max(normalized_log2) * 1.2, 
                   xend = "Tamoxifen", yend = max(normalized_log2) * 1.2), 
               colour = "black", linetype = 1, size = 0.7) +
  geom_text(aes(x = 1.5, y = max(normalized_log2) * 1.3, 
                label = ifelse(get_pval(pooled_aov) < 0.01, "p < 0.01", get_pval(pooled_aov)))) +
  scale_y_continuous(name = "log2 flod change") +
  scale_x_discrete(name = NULL) +
  theme_classic() +
  theme(axis.text = element_text(face = "bold"))

ggsave("G:/data/live-image/PB3-Pdgfb.tiff")
```
...
ggsave("G:/data/kidney/Tumor/PB3/res-microscope/F480-positve-area.tiff", 
       width = 10, height = 7)
```
...
cd31_junction_corn %>% bind_rows(cd31_junction_tam) %>% 
  ...
  # geom_density_ridges(aes(x = junctions_density, y = group, fill = group))
  geom_boxplot() +
  geom_jitter(width = 0.1) +
  geom_segment(aes(x = 1, y = max(junctions_density) * 1.1, 
                   xend = 2, yend = max(junctions_density) * 1.1), 
               size = 1) +
  geom_text(aes(x = 1.5, y = max(junctions_density) * 1.15, label = "p < 0.05")) +
...
ggsave("G:/data/kidney/Tumor/PB3/res-microscope/junctions_density.tiff", width = 10, height = 7)
```
...
cd31_area_corn %>% bind_rows(cd31_area_tam) %>% 
  mutate(group = ifelse(grepl("TAM", label), "Tamoxifen", "Corn Oil")) %>>%
  (~ cd31_area) %>% 
  ggplot(aes(group, x_area)) +
  geom_boxplot() +
  geom_jitter(size = 1.5, width = 0.1) +
  geom_segment(aes(x = 1, y = max(x_area) * 1.2, 
                   xend = 2, yend = max(x_area) * 1.2), 
               size = 1) +
  geom_text(aes(x = 1.5, y = max(x_area) * 1.24, label = "ns")) +
  theme_classic() +
  theme(axis.text = element_text(face = "bold", size = 13), 
        axis.title = element_text(face = "bold", size = 15)) +
  scale_y_continuous(name = "percentage of CD31 \npositive area", 
                     breaks = c(2, 4, 6), 
                     labels = paste(c(2, 4, 6), "%", sep = "")) +
  scale_x_discrete(name = NULL)
...
```{r}
asma_pdgfrb <- read.csv("G:/data/kidney/Tumor/PB3/kidney-PB3-WT/a-SMA-Pdgfrb/prob-maps/PB3-WT-a-SMA-Pdgfrb-colocalization.scv", 
                        sep = "\t") %>% 
  clean_names()

pdgfrb <- asma_pdgfrb %>% 
  filter(grepl("^Classification.+-0001", label)) %>% 
  select(label_pdgfrb = label, x_area_pdgfrb = x_area)

double <- asma_pdgfrb %>% 
  filter(grepl("^Result", label)) %>% 
  select(label_double = label, x_area_double = x_area)

plot_data_asma_pdgfrb <- pdgfrb %>% cbind(double) %>% 
  mutate(frac = round(x_area_double / x_area_pdgfrb, 3)) 

plot_data_asma_pdgfrb %>% 
  ggplot(aes("a-SMA", frac)) +
  geom_boxplot() +
  geom_jitter(size = 3, width = 0.1) +
  scale_x_discrete(name = NULL) +
  scale_y_continuous(name = NULL, 
                     breaks = seq(0.1, 0.4, 0.1), 
                     labels = paste(seq(10, 40, 10), "%", sep = ""), 
                     limits = c(0.1, 0.4), 
                     expand = c(0, 0)) +
  labs(title = "Double positive area to Pdgfrb positive area") +
  theme_classic()

ggsave("G:/data/kidney/Tumor/PB3/kidney-PB3-WT/a-SMA-Pdgfrb/prob-maps/double-positive-area-to-pdgfrb-positive-area.tiff", 
       width = 5, height = 7)
```
...
### CD45, Pdgfrb
```{r}
cd45_pdgfrb <- read.csv("G:/data/kidney/Tumor/PB3/kidney-PB3-WT/CD45-Pdgfrb/prob-maps/PB3-WT-CD45-Pdgfrb-colocalization.csv") %>% 
  clean_names()

pdgfrb <- cd45_pdgfrb %>% 
  filter(grepl("^Probability-.+-0001", label)) %>% 
  select(label_pdgfrb = label, x_area_pdgfrb = x_area)
...
plot_data_cd45_pdgfrb <- cbind(double, pdgfrb) %>% 
  mutate(frac = round(x_area_double / x_area_pdgfrb, 3))

plot_data_cd45_pdgfrb %>% 
  ggplot(aes("CD45", frac)) +
  geom_boxplot() +
  geom_jitter(size = 3, width = 0.1) +
  scale_y_continuous(breaks = seq(0.1, 0.4, 0.1), 
                     ...
                     limits = c(0.1, 0.4)) +
  labs(x = NULL, y = NULL, 
       title = "Double positive area to Pdgfrb positive area") +
  theme_classic()

ggsave("G:/data/kidney/Tumor/PB3/kidney-PB3-WT/CD45-Pdgfrb/prob-maps/doubel-positive-area-to-pdgfrb-positive-area.tiff", width = 5, height = 7)
```
