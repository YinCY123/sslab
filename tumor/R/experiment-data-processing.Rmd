...
pooled %>% 
  ...
  geom_jitter(aes(group, normalized_log2), width = 0.1, size = 4) +
...
ggsave("G:/data/kidney/Tumor/PB3/res-microscope/a-SMA-intensity-plot.tiff", 
       width = 10, height = 7)
```
...
rbind(pb3_corn_oil_asma, pb3_tam_asma) %>% 
  ...
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.2) +
  geom_jitter(aes(y = x_area),width = 0.1, size = 4) +
  geom_segment(aes(x = 1, y = max(x_area) * 1.1, 
                   xend = 2, yend = max(x_area) * 1.1)) +
  geom_text(aes(x = 1.5, y = max(x_area) * 1.14), label = "***", size = 10) +
  scale_x_discrete(name = NULL) +
...
write.csv(f480[, -4], "G:/data/kidney/Tumor/PB3/res-microscope/F480-cells-per-region.csv")
...
f480 %>% 
  ...
  geom_bar(aes(group, mean), stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(x = group, y = mean, ymin = mean - se, ymax = mean + se), width = 0.2) +
  geom_jitter(aes(group, num_cells), width = 0.1, size = 4) +
  geom_segment(aes(x = 1, y = max(num_cells) * 1.1, 
                   xend = 2, yend = max(num_cells) * 1.1)) +
  geom_text(aes(x = 1.5, y = max(num_cells) * 1.15), label = "ns", size = 10) +
  scale_x_discrete(name = NULL) +
  scale_y_continuous(name = "number of F480 positive \ncells per view") +
  my_theme

ggsave("G:/data/kidney/Tumor/PB3/res-microscope/F480-cells-per-region.tiff", 
       width = 7, height = 7)
```
...
cd31_junction_corn %>% bind_rows(cd31_junction_tam) %>% 
  ...
  geom_segment(aes(x = 1, y = max(junctions_density) * 1.1, 
               ...
               size = 1) +
  geom_text(aes(x = 1.5, y = max(junctions_density) * 1.12), label = "*", size = 10) +
...
ggsave("G:/data/kidney/Tumor/PB3/res-microscope/CD31-junctions-density.tiff", width = 7, height = 10)
```
...
cd31_area_corn %>% bind_rows(cd31_area_tam) %>>% 
  ...
  ggplot() +
  geom_bar(aes(group, mean), stat = "identity", position = position_dodge()) +
  geom_jitter(aes(group, x_area), width = 0.1, size = 4) +
  geom_segment(aes(x = 1, y = max(x_area) * 1.2, 
                   xend = 2, yend = max(x_area) * 1.2), 
               size = 1) +
  geom_text(aes(x = 1.5, y = max(x_area) * 1.25), label = "ns", size = 10) +
...
ggsave("G:/data/kidney/Tumor/PB3/res-microscope/CD31-positive-area.tiff", 
       width = 7, height = 10)
```
...
pdgfrb_area_corn_oil %>% rbind(pdgfrb_area_tam) %>>%
  ...
  geom_segment(aes(x = 1, y = max(x_area) * 1.1, 
                   xend = 2, yend = max(x_area) * 1.1)) +
  geom_text(aes(x = 1.5, y = max(x_area) * 1.12), label = "*", size = 10) +
  scale_x_discrete(name = NULL) +
  scale_y_continuous(name = "Percentage of Pdgfrb \nPositive Area") +
  my_theme

pdgfrb_are_aov <- aov(x_area ~ group, data = pdgfrb_area) %>% summary
ggsave("G:/data/kidney/Tumor/PB3/res-microscope/Pdgfrb-positive-area.tiff", 
       width = 7, height = 10)
```
...
pdgfrb_cell_corn_oil %>% rbind(pdgfrb_cell_tam) %>>%
  ...
  geom_segment(aes(x = 1, y = max(num_cells) * 1.1, 
                   xend = 2, yend = max(num_cells) * 1.1)) +
  geom_text(aes(x = 1.5, y = max(num_cells) * 1.12), label = "**", size = 10) +
  scale_x_discrete(name = NULL) +
  scale_y_continuous(name = "Number of Pdgfrb Positive \nCells per View") +
  my_theme

aov(num_cells ~ group, data = pdgfrb_cells) %>% summary
ggsave("G:/data/kidney/Tumor/PB3/res-microscope/Pdgfrb-cells-per-view.tiff", 
       width = 7, height = 10)
```
...
asma_pdgfrb_coloc %>%
  ...
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.2) +
  geom_jitter(aes(y = spearman_rank_corelation), width = 0.1, size = 4) +
  scale_y_continuous(name = "Spearman Rank Corelation", limits = c(0, 0.45)) +
  scale_x_discrete(name = NULL) +
  ggtitle(label = "aSMA Pdgfrb colocalization coefficiency") +
  my_theme
...
cd45_pdgfrb_coloc %>% 
  ...
  ggtitle(label = "CD45 Pdgfrb colocalization coefficiency") +
  my_theme
